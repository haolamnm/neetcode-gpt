...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x: TensorType[float]) -> TensorType[float]:
        ...
        # Output shape: (B, num_heads/num_kv_heads, T, head_dim)
        ...
        scores = torch.matmul(q, k.transpose(-2, -1)) / torch.sqrt(torch.tensor(self.head_dim, dtype=x.dtype, device=x.device))
